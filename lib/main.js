...
/*
              ...
              var options = historyService.getNewQueryOptions();
              var result = historyService.executeQuery(query, options);
              var containerResultNode = result.root;
*/
...
// Press this widget to open folders tab:
var foldersWidget = widget.Widget({
    id: "foldersWidget",
   ...
   worker.port.on("gotText", function (pageInfoJSON) {
     ...
     if (!bookmarksService.getBookmarkedURIFor(uri)) { // don't bookmark URI multiple times
      dict.listitems().forEach(function (item) {
        ...
        var matchStrings = pageInfo.text.match(new RegExp(matchExpression, "igm"));
        if (matchStrings) {
         // Use matching strings as tags, for easier future reference:
         for (var k = 0; k < matchStrings.length; k++) {
          if (matchStrings[k])
           taggingSvc.tagURI(uri, [matchStrings[k]]);
         }
